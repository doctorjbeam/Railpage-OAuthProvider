<?php
	...
	class Token implements IToken{
		...
		public static function findByToken($token){
			$ret = null;
			$pdo = Db::singleton();
			$find = $pdo->query("select id from oauth_token where token = '".$token."'");
			if($find->rowCount()==1){
				$find = $find->fetch();
		...
		private function load(){
			$info = $this->pdo->query("select * from oauth_token where id = ".$this->id)->fetch();
		...
		public function changeToAccessToken($token,$secret){
			if($this->isRequest()){
				$this->pdo->exec("update oauth_token set type = 2, verifier = '', callback_url = '', token = '".$token."', token_secret = '".$secret."' where id = ".$this->id);	
				return true;
			} else {
				return false;
			}
		...
		public function setVerifier($verifier){
			$this->pdo->exec("update oauth_token set verifier = '".$verifier."' where id = ".$this->id);
			$this->verifier = $verifier;
		}
		
		public function setUser(IUser $user){
			$this->pdo->exec("update oauth_token set consumer_id = '".$user->getId()."' where id = ".$this->id);
			$this->user = $user;
		}
